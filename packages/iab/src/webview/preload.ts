...
export interface IABAPI {
  ...
  selectElement: (selector: string) => Promise<{ tag: string; text: string; html: string; rect: DOMRect } | null>
...
const api: IABAPI = {
  ...
  selectElement: async (selector) => {
    return ipcRenderer.invoke("iab-select-element", selector)
  },

  getComputedStyles: async (selector) => {
    return ipcRenderer.invoke("iab-get-computed-styles", selector)
  },
}
